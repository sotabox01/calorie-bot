...
2. Если вес указан явно — ВСЕГДА используй его. weight_type = "exact".
   ...
   - Яйцо — 1 шт (~60г)
   - Хлеб — 1 кусок (~30г)
3. Если пользователь указал БЖУ явно (например "7.3 жира", "30г белка"), ИСПОЛЬЗУЙ эти значения.
4. КРИТИЧНО: Если пользователь написал "Xг продукта (Y ккал)", то Y — это ОБЩАЯ калорийность для X граммов, а не на 100г. Посчитай КБЖУ на 100г пропорционально: на 100г = Y * 100 / X. Но в ответе пиши weight_g = X, а kcal = Y (общая).
5. Если в сообщении нет еды (приветствие, вопрос и т.п.) — верни пустой список items.
6. Возвращай ТОЛЬКО JSON. Никакого другого текста, пояснений, рассуждений.
